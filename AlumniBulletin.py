...
TAGS_WANTED = {'language', 'subject', 'volume', 'date', 'call_number', 'publisher', 'description',
...
replaceList = {'language': 'Language-Code', 'subject': 'Subject-Topic', 'volume': 'Volume-Number',
               'date': 'Date-Created', 'call_number': 'Call-Number', 'publisher': 'Creator',
               'description': 'Description-Abstract',
               'possible-copyright-status': 'Rights', 'mediatype': 'Type', 'identifier': 'Identifier', 'ppi': 'PPI',
...
itemList = 'S:/Digital Projects/Internet Archive Downloads/Alumni Bulletin/alumni_bulletin_itemlist.txt'  # itemlist directory (CHANGE)
...
xmlDP = 'S:/Digital Projects/Internet Archive Downloads/Alumni Bulletin/XML'  # String to capture a longer directory address cause API call has some restrictions (CHANGE)
...
for i in range(txtLength):
    ...
    csvPath = 'S:/Digital Projects/Internet Archive Downloads/Alumni Bulletin/CSV/'  # (CHANGE) directory path for where CSV file is located
    ...
    parent_object = "digitalcollections:alumni-bulletins"
    ...
    for child in root:  # same as for loop up top, except it appends the element instead
        ...
        if tag == 'scandate':  # TEST
            ...
            if tag == 'possible-copyright-status':
                CSV_Main.append("This work is licensed under a Creative Commons Atrribution-NonCommerical-NoDerivs 3.0 Unported License.")
            else:
    ...
    scanDataDP = 'S:/Digital Projects/Internet Archive Downloads/Alumni Bulletin/scandata'  # (CHANGE) DIRECTORY
    ...
    SDFolderPath = 'S:/Digital Projects/Internet Archive Downloads/Alumni Bulletin/' # (CHANGE)
...
#notes: Alumni Bulletin, volume 14 issue 01 is corrupted, the script is not ran with it, might have to redownload the zip and extract later and re-run
