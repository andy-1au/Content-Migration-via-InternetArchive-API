...
replaceList = {'language': 'Language-Code', 'subject': 'Subject-Topic', 'volume': 'Volume-Number',
               ...
               'scandate': 'Date-Captured'}  # dictionary similar to set above, used to replace header array elements with more specific rules
                                             # (CHANGE) the 'date' or 'pub_date' tag

additionalHeadersList = ["pid", "parent_object", "cmodel", "object_location", "label", "Title", "Type", "Year",
                         "Language-Code", "Language-Text", "Format", "Format-URL", "File-Format", "Digital-Origin",
                         "Rights"]  # adding the other needed headers

itemList = 'S:/Digital Projects/Internet Archive Downloads/Auf Deutsch Bitte/auf_deutsch_bitte_itemlist.txt'  # itemlist directory (CHANGE)
...
xmlDP = 'S:/Digital Projects/Internet Archive Downloads/Auf Deutsch Bitte/XML'  # String to capture a longer directory address cause API call has some restrictions (CHANGE)
...
for i in range(txtLength):
    ...
    csvPath = 'S:/Digital Projects/Internet Archive Downloads/Auf Deutsch Bitte/CSV/'  # (CHANGE) directory path for where CSV file is located
    ...
    for child in root:  # same as for loop up top, except it appends the element instead
        ...
        if tag == 'scandate':  # TEST
            ...
            if tag == 'pub_date': # (CHANGE) date tag may be different
                year = child.text
    ...
    scanDataDP = 'S:/Digital Projects/Internet Archive Downloads/Auf Deutsch Bitte/scandata'  # (CHANGE) DIRECTORY
    ...
    SDFolderPath = 'S:/Digital Projects/Internet Archive Downloads/Auf Deutsch Bitte/' # (CHANGE)
    ...
    folderName_add = "Auf Deutsch Bitte" # (CHANGE)
