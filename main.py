...
TAGS_WANTED = {}  # SET used to search for specific tags in XML file 
        
replaceList = {}  # dictionary similar to set above, used to replace header array elements with more specific rules
...
additionalHeadersList = ["pid", "parent_object", "cmodel", "object_location", "label", "Title", "Type", "Year",
                         ...
                         "Rights"]  # adding the other needed headers in this specific order

itemList = ''  # CHANGE DIRECTORY | itemlist directory
...
xmlDP = ''  # CHANGE DIRECTORY | String used to capture a longer directory address cause API call has some restrictions 
...
for i in range(txtLength):
    ...
    csvPath = ''  # CHANGE DIRECTORY | directory path for where CSV file is located
    ...
    # Write Headers ---------------------
    CSV_Data = open(fullCSVPath, 'w', encoding='utf-8', newline='') # creates a new CSV file for writing with parameters
                                                                    # newline is made so that there aren't any empty lines after what is written
                                                                    # encoding matches what is on the XML File
    CSVWriter = csv.writer(CSV_Data)  # object used from the CSV class to write into the specify CSV path 
    CSV_Header = []  # creates a list for the header of the CSV File

    for child in root:  # loop to go through every tag in XML file and matches with the set
    ...
    for child in root:  # same as loop up top, except it appends the element instead
        tag = child.tag
        if tag == 'scandate':  # scan data format in XML files are different, changing it here 
...
#         elif tag == 'title': # use this for the additional elements that require repetition of the tag's info 
#             label = child.text
#             title = child.text
        elif tag in TAGS_WANTED:
            CSV_Main.append(child.text)
            if tag == 'title': # use this for the additional elements that require repetition of the tag's info 
            label = child.text
            title = child.text
            # in this section below, date is generally different and not standard in the xml files 
            if tag == 'date': # change 'date' here if it's different 
                year = child.text
    ...
    # Remove certain pictures in the scanned photos folders
    scanDataDP = ''  # CHANGE DIRECTORY | directory path to scandata folder
    download(lines[i], verbose=True, glob_pattern='*scandata.xml', destdir=scanDataDP, ignore_existing=True, no_directory=True)

    SDFileName = lines[i] + '_scandata.xml'  # Adds xml extension for file reading reasons
    fullSDPath = scanDataDP + '/' + SDFileName # Path where the scandata is held 
    SDFolderPath = '' # CHANGE DIRECTORY | Main folder where the photos are held
    ...
    folderName_add = '' # CHANGE Folder Name 
    ...
    for j in range(len(imagesList)):
        object_location_add = 'root_folder' + folderName_add + '/' + SDFolderFileName + SDFolderFileName + imagesList[j] 
